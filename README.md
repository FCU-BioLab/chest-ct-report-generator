...
一個完整的端到├── 🔬 Fine_Tune/               # 模型微調
│   └── llama3.2/              # LLaMA微調配置
├── 📊 dataset_splits/          # 資料集分割結果
│   ├── train/                 # 訓練集動的胸部CT影像分析和醫療報告生成系統，整合了CT-ViT目標檢測、RAG知識檢索和智能報告生成功能。
...
- **資料集分割**: `dataset_splitter.py` - 智能化資料集劃分
- **DICOM查看**: `dicom_viewer.py` - 醫學影像查看與分析
- **文件管理**: `copy_matched_files.py`, `copy_all_patients_matched_files.py` - 數據文件複製與組織

#### 3. 原始資料處理模組 (original_datasets/)
本專案使用 **Lung-PET-CT-Dx** 資料集，以下程式碼負責處理原始數據：
...
##### 🔧 核心處理程式
- **單一患者處理**: [`CT_ViT_Training/tools/copy_matched_files.py`](CT_ViT_Training/tools/copy_matched_files.py)
  - 處理指定患者的DICOM-XML文件匹配
  - 基於SOP Instance UID進行文件配對
  - 從原始資料集提取有效的標註配對

- **批量患者處理**: [`CT_ViT_Training/tools/copy_all_patients_matched_files.py`](CT_ViT_Training/tools/copy_all_patients_matched_files.py) 
  - 遍歷所有352位患者（A:248, B:38, E:5, G:61）
  - 自動匹配每位患者的DICOM影像與XML標註
  - 生成處理統計報告和患者資料總覽
...
##### 📊 數據處理流程
```
原始資料集 (original_datasets/)
    ↓ copy_matched_files.py
匹配數據資料 (matched_data_by_patient/)  
    ↓ dataset_splitter.py
...
```python
# 從DICOM文件提取UID
dcm_uid = pydicom.dcmread(dcm_path).SOPInstanceUID

# 從XML文件名提取UID  
xml_uid = os.path.basename(xml_path).replace('.xml', '')

# 基於UID進行精確匹配
if dcm_uid == xml_uid:
    matched_pairs.append({'dcm': dcm_path, 'xml': xml_path})
```
...
# 查看患者數據結構
python tools/dataset_splitter.py --info
```

### 3. 原始資料處理（首次設置）

如果有原始的 **Lung-PET-CT-Dx** 資料集，需要先進行資料前處理：

```powershell
# 處理單一患者的DICOM-XML配對 (測試用)
cd CT_ViT_Training\tools
python copy_matched_files.py

# 批量處理所有患者數據 (完整處理)
python copy_all_patients_matched_files.py
```

**資料來源路徑配置**（需要修改腳本中的路徑）:
```python
# DICOM影像路徑
search_path = r'original_datasets\Lung-PET-CT-Dx\manifest-1608669183333\Lung-PET-CT-Dx\Lung_Dx-{patient_id}'

# XML標註路徑  
xml_search_path = r'original_datasets\Lung-PET-CT-Dx\Lung-PET-CT-Dx-Annotations-XML-Files-rev12222020\Annotation\{patient_id}'

# 輸出路徑
output_dir = r'matched_data_by_patient\{patient_id}'
```

**處理統計**:
- 📊 **352位患者**: A系列248位, B系列38位, E系列5位, G系列61位
- 🔗 **30,382組配對**: DICOM影像與XML標註精確匹配
- ✅ **匹配準確度**: 基於SOP Instance UID的100%精確匹配
